...
```{r}
test<-garch_fit@fit$coef
```
```{r}
arima_model <- arima(btc_returns$Return,
                     order = c(1, 0, 1),  # ARIMA(p, d, q)
                     fixed = c(test["ar1"], test["ma1"], test["mu"]))  # Use coefficients
summary(arima_model)

predictions <- predict(arima_model, n.ahead = 322)
```

```{r}
errors <- btc_data_new$Return - predictions$pred 
...
```
```{r}
fit <- Arima(btc_returns$Return, order=c(3,1,1))
fit
```
```{r}
returnlist <-  fit$residuals
garch_spec <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1,1)),
                         distribution.model = "norm"	,
                         mean.model = list(armaOrder = c(3,1,1)))
garch_fit <- ugarchfit(spec = garch_spec, data = btc_returns$btc_usd_close)
print(garch_fit)
```
