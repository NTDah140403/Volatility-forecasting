...
```{r}
volatilityGarch <- data.frame(coredata(eth_return), VolatilityGarch = sigma(def.fit))
...
results <- results |> 
  mutate(
    Upper_Bound = uncmean(def.fit) + qnorm(0.95) * lag(results$Predicted_Volatility),  # Upper bound
    Lower_Bound = uncmean(def.fit) - qnorm(0.95) * lag(results$Predicted_Volatility)   # Lower bound
  )

# Create the plot
ggplot(results, aes(x = Date)) +
  geom_line(aes(y = Actual_Return, color = "Return"), size = 0.8) +
  geom_line(aes(y = Upper_Bound, color = "Upper Bound"), linetype = "dashed", size = 0.8) +
  geom_line(aes(y = Lower_Bound, color = "Lower Bound"), linetype = "dashed", size = 0.8) +
  labs(
